...
const createTokenAndMintTo = async (
  ...
  let mintToIxs = mintTo.flatMap(({ recepient, amount }) => {
    ...
    return [
      createAssociatedTokenAccountIdempotentInstruction(payer, ataAddress, recepient, tokenMint, TOKEN_PROGRAM),
...
describe('escrow', () => {
  ...
  const makeOfferTx = async (
    ...
    const vaultAddress = getAssociatedTokenAddressSync(offeredTokenMint, offerAddress, true, TOKEN_PROGRAM);

    return { offerAddress, vaultAddress };
  };

  const closeOfferTx = async (offerAddress: PublicKey, maker: Keypair): Promise<void> => {
    const transactionSignature = await program.methods
      .closeOffer()
      .accounts({
        maker: maker.publicKey,
  ...
  test('Maker closes offer, vault is closed and tokens returned', async () => {
    const offeredUsdc = new BN(10_000_000);
    const wantedWif = new BN(100_000_000);
    ...
    // Check Alice's balance before closing
    const aliceBalanceBefore = await getTokenBalance(aliceUsdcAccount);
    expect(await getTokenBalance(vaultAddress)).toEqual(offeredUsdc);

    // Close the offer
    await closeOfferTx(offerAddress, alice);

    // Vault should be closed (fetch should fail)
    await expect(connection.getTokenAccountBalance(vaultAddress)).rejects.toThrow();

    // Offer account should be closed (fetch should fail)
    await expect(program.account.offer.fetch(offerAddress)).rejects.toThrow();

    // Alice's balance should have increased by the offered amount
    const aliceBalanceAfter = await getTokenBalance(aliceUsdcAccount);

    expect(aliceBalanceAfter.sub(aliceBalanceBefore)).toEqual(offeredUsdc);
  });
